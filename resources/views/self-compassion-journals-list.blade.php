...
<div x-data="selfCompassionJournalListApp()" x-init="init()" x-cloak>
    ...
    <div class="space-y-3">
        ...
        <template x-for="item in journals" :key="item.id">
            <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
                <button
                    ...
                    class="w-full text-left p-4 hover:bg-rose-50 transition-colors"
                >
                ...
                <div x-show="expandedId === item.id" x-collapse class="border-t border-gray-100">
                    <div class="p-4 space-y-4 bg-rose-50/50">
                        @foreach ($fieldLabels as $key => $label)
                            <div>
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-rose-500 text-white text-[10px] font-bold">{{ $loop->iteration }}</span>
                                    <span class="text-xs font-semibold text-rose-700">{{ $label }}</span>
                                </div>
        ...
        <div x-show="!loading && journals.length === 0" class="text-center py-12 text-gray-500">
            ...
            <p>まだ記録がありません</p>
            <a href="/self-compassion-journals" class="text-rose-600 hover:text-rose-800 text-sm mt-4 inline-block">
