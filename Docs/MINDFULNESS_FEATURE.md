...
これらの定義は `App\Enums\MindfulnessSound` 列挙型で一元管理されています。
...
これらの定義は `App\Enums\MindfulnessDuration` 列挙型で一元管理されています。
...
9ファイル合計で `.wav` だと数GB、`.mp3` なら数百MBに収まる。
R2の保存料金・ブラウザの読み込み速度の観点から `.mp3` が望ましい。

### 変換コマンド（ffmpeg）

```bash
ffmpeg -i forest_5m.wav -codec:audio libmp3lame -qscale:a 2 forest_5m.mp3
```

### ファイル命名規則

```
{音の種類ID}_{再生時間（分）}m.wav
```

> **注意**: 現在の実装では `.wav` 形式を使用しています。今後 `.mp3` への移行を予定しています（[Issue #91](https://github.com/class-kouta/cbt_app/issues/91) 参照）。
...
例: `https://pub-xxxxxxxxxxxxxxxx.r2.dev/mindfulness/forest_5m.wav`
...
例: `http://localhost:8081/mindfulness/forest_5m.wav`
